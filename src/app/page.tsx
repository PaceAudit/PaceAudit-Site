...
const css = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .day-date { font-size: 11px; color: var(--text2); font-weight: 400; margin-top: 2px; }
  .schedule-card {
    background: var(--surface2); border: 1px solid var(--border);
  ...
  .schedule-card .title { font-weight: 600; color: var(--text); margin-bottom: 4px; }
  .schedule-card .meta { font-size: 11px; color: var(--text3); }
  .schedule-card.blog { border-left: 3px solid var(--accent); }
  .schedule-card.linkedin { border-left: 3px solid #0a66c2; }
  .schedule-card.twitter { border-left: 3px solid #1da1f2; }
  .schedule-card.facebook { border-left: 3px solid #1877f2; }
  .schedule-card.instagram { border-left: 3px solid #e4405f; }
  .empty-day { flex: 1; display: flex; align-items: center; justify-content: center; color: var(--text3); font-size: 12px; }
`;
...
function ConfigPage() {
  ...
  const [props, setProps] = useState([
    ...
    { id: 4, text: "One-click publishing to Netlify, LinkedIn, Facebook, Meta (Instagram), and X" },
  ]);
        ...
        <div className="card">
          ...
          {props.map((p) => (
            <div className="prop-item" key={p.id}>
              ...
              <input type="text" value={p.text} onChange={e => updateProp(p.id, e.target.value)} placeholder="Enter a value proposition..." />
...
// Mock scheduled content for calendar views (in real app, fetch from API by scheduled_date)
...
const mockBlogSchedule: { title: string; date: string; time: string }[] = [
  { title: "How to Build a Content Moat with AI", date: "2025-02-24", time: "09:00" },
  { title: "Top 10 AI Tools for Startups in 2025", date: "2025-02-26", time: "10:00" },
  { title: "The Hidden Costs of Manual Content Creation", date: "2025-02-28", time: "09:00" },
];
const mockSocialSchedule: { title: string; channel: string; date: string; time: string }[] = [
  { title: "Content Moat with AI", channel: "LinkedIn", date: "2025-02-24", time: "12:00" },
  { title: "Content Moat with AI", channel: "Twitter", date: "2025-02-24", time: "14:00" },
  { title: "Content Moat with AI", channel: "LinkedIn", date: "2025-02-25", time: "09:00" },
  { title: "AI Tools for Startups", channel: "LinkedIn", date: "2025-02-26", time: "11:00" },
  { title: "AI Tools for Startups", channel: "Twitter", date: "2025-02-27", time: "10:00" },
  { title: "Hidden Costs of Manual Content", channel: "Instagram", date: "2025-02-28", time: "15:00" },
];

function BlogScheduleView() {
  ...
  const byDay = days.map(d => {
    ...
    return mockBlogSchedule.filter(b => b.date === key);
  });
  return (
    <div>
      <div style={{ marginBottom: 16, display: "flex", alignItems: "center", gap: 10 }}>
        <Icon d={icons.calendar} size={18} />
        <span style={{ fontSize: 13, color: "var(--text2)" }}>Finalize your blog schedule here. Content in Review can be approved and assigned to a day.</span>
      </div>
      <div className="week-grid">
        {days.map((d, i) => (
          <div key={i} className="day-column">
            <h3>{weekDayNames[d.getDay()]}</h3>
            <span className="day-date">{d.toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}</span>
            <div style={{ flex: 1, marginTop: 10 }}>
              {byDay[i].length === 0 ? (
                <div className="empty-day">No blogs</div>
              ) : (
                byDay[i].map((b, j) => (
                  <div key={j} className="schedule-card blog">
                    <div className="title">{b.title}</div>
                    <div className="meta">{b.time} · Blog</div>
                  </div>
                ))
              )}
            </div>
...
function TopicsPage() {
  ...
  const [topicsTab, setTopicsTab] = useState<"database" | "schedule">("database");
  ...
  const handleGenerate = (id: number) => {
    ...
    setTimeout(() => {
      ...
      setGenerating(null);
      setTimeout(() => setTopics(t => t.map(x => x.id === id ? { ...x, status: "Review" } : x)), 2000);
    }, 600);
  };
    ...
    <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
      ...
      <div className="content">
        <div className="tab-bar">
          <button className={`tab-item${topicsTab === "database" ? " active" : ""}`} onClick={() => setTopicsTab("database")}>
            Topic Database
          </button>
        ...
        <div style={{ display: "flex", gap: 12, marginBottom: 20, flexWrap: "wrap" }}>
          {["All", "Pending", "Generating", "Review", "Published"].map(s => (
            <button key={s} className="btn btn-ghost" style={{ fontSize: 12 }}>{s}
              {s !== "All" && <span style={{ marginLeft: 4, background: "var(--border)", padding: "1px 6px", borderRadius: 99, fontSize: 10 }}>{topics.filter(t => t.status === s).length}</span>}
            </button>
          ))}
        </div>
                ...
                <tr key={t.id}>
                  ...
                  <td>
                    <div className="actions">
                      {t.status === "Pending" && (
                        <button className="btn btn-primary" style={{ padding: "5px 10px", fontSize: 12 }} onClick={() => handleGenerate(t.id)} disabled={generating === t.id}>
                          <Icon d={icons.spark} size={13} />
                          {generating === t.id ? "Queuing…" : "Generate"}
                        </button>
                      )}
                      {t.status === "Review" && <button className="btn btn-secondary" style={{ padding: "5px 10px", fontSize: 12 }}>Review →</button>}
                      <button className="btn btn-ghost" style={{ padding: "5px 8px" }}><Icon d={icons.trash} size={13} /></button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Add New Topic">
        <div className="form-group">
          ...
          <input type="text" value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} placeholder="e.g. How AI is Reshaping B2B Sales in 2025" />
        </div>
        <div className="form-group">
          <label className="form-label">Target Keyword</label>
          <input type="text" value={form.keyword} onChange={e => setForm(f => ({ ...f, keyword: e.target.value }))} placeholder="e.g. ai b2b sales tools" />
        ...
        <div className="form-group">
          ...
          <input type="text" value={form.angle} onChange={e => setForm(f => ({ ...f, angle: e.target.value }))} placeholder="e.g. Contrarian take — AI won't replace reps, it'll amplify them" />
        </div>
        <div className="form-group">
          <label className="form-label">Target Persona</label>
          <input type="text" value={form.persona} onChange={e => setForm(f => ({ ...f, persona: e.target.value }))} placeholder="e.g. VP of Sales at a 50-200 person B2B SaaS company" />
        </div>
        <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 8 }}>
          <button className="btn btn-secondary" onClick={() => setModalOpen(false)}>Cancel</button>
          <button className="btn btn-primary" onClick={handleAdd}><Icon d={icons.plus} size={14} /> Add Topic</button>
        </div>
      </Modal>
    </div>
  );
        ...
        <div className="review-grid">
          <div>
            ...
            <div className="form-group" style={{ marginBottom: 0 }}>
              ...
              <textarea rows={22} value={blog} onChange={e => setBlog(e.target.value)} style={{ fontFamily: "monospace", fontSize: 12.5, lineHeight: 1.6 }} />
            ...
            <div className="card" style={{ marginTop: 20, padding: 16 }}>
              ...
              <div className="form-group" style={{ marginBottom: 0 }}>
                ...
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  {["Netlify (website)", "LinkedIn", "Facebook", "Meta (Instagram)", "X (Twitter)"].map(ch => (
...
function SocialScheduleView() {
  ...
  const byDay = days.map(d => {
    const key = d.toISOString().slice(0, 10);
    return mockSocialSchedule.filter(s => s.date === key);
  });
  const channelClass: Record<string, string> = { LinkedIn: "linkedin", Twitter: "twitter", Facebook: "facebook", Instagram: "instagram" };
  return (
    <div className="week-grid">
      {days.map((d, i) => (
        <div key={i} className="day-column">
          ...
          <div style={{ flex: 1, marginTop: 10 }}>
            {byDay[i].length === 0 ? (
              <div className="empty-day">No posts</div>
            ) : (
              byDay[i].map((s, j) => (
                <div key={j} className={`schedule-card ${channelClass[s.channel] || ""}`}>
                  <div className="title">{s.title}</div>
                  <div className="meta">{s.time} · {s.channel}</div>
                </div>
              ))
            )}
          </div>
...
function SocialMediaPage() {
  const [socialTab, setSocialTab] = useState<"generate" | "schedule">("generate");
  const [generating, setGenerating] = useState(false);
  const [toast, setToast] = useState(false);
  const runGenerate = () => {
    setGenerating(true);
    setTimeout(() => { setGenerating(false); setToast(true); setTimeout(() => setToast(false), 3000); }, 1500);
  };
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
      <div className="topbar">
        <div>
          <div className="topbar-title">Social Media</div>
          <div className="topbar-sub">Content derived from your blog. Finalize the blog schedule first, then generate all social copy from those posts.</div>
        </div>
      </div>
      <div className="content">
        <div className="tab-bar">
          <button className={`tab-item${socialTab === "generate" ? " active" : ""}`} onClick={() => setSocialTab("generate")}>
            <Icon d={icons.spark} size={14} style={{ marginRight: 6, verticalAlign: "middle" }} />
            Generate from blogs
          </button>
          <button className={`tab-item${socialTab === "schedule" ? " active" : ""}`} onClick={() => setSocialTab("schedule")}>
        ...
        {socialTab === "generate" ? (
          <div className="card">
            <div className="card-title">Generate social content from blogs</div>
            <div className="card-desc" style={{ marginBottom: 20 }}>
              Use this after you’ve finalized your blog schedule. We’ll generate LinkedIn, X (Twitter), Facebook, and Instagram copy derived from each scheduled blog post so you can review and then assign days in the Social Schedule.
            </div>
            <div style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
              <button className="btn btn-primary btn-large" onClick={runGenerate} disabled={generating}>
                <Icon d={icons.spark} size={18} />
                {generating ? "Generating…" : "Generate all from blogs"}
              </button>
              <span style={{ fontSize: 12.5, color: "var(--text3)" }}>
                Runs across all scheduled blog content and creates social variants.
              </span>
            </div>
          </div>
        ) : (
          <div>
            <div style={{ marginBottom: 16, display: "flex", alignItems: "center", gap: 10 }}>
              <Icon d={icons.calendar} size={18} />
              <span style={{ fontSize: 13, color: "var(--text2)" }}>Week at a glance — what’s going out on each day (LinkedIn, X, Facebook, Instagram).</span>
            </div>
            <SocialScheduleView />
          </div>
        )}
      </div>
      {toast && <Toast msg="Social content generated from blogs" />}
    </div>
  );
}
...
export default function App() {
  ...
  const renderPage = () => {
    ...
    if (page === "social") return <SocialMediaPage />;
  };
        ...
        <aside className="sidebar">
          ...
          <div style={{ borderTop: "1px solid var(--border)", paddingTop: 12, marginTop: 8 }}>
            <div style={{ fontSize: 11.5, color: "var(--text3)", padding: "0 10px" }}>
              <div style={{ fontWeight: 600, color: "var(--text2)", marginBottom: 2 }}>AI Content Engine</div>
